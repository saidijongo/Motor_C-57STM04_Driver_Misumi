...
void runPumps(int runTime)
{
  ...
  Serial.println("Running all pumps");
}
...
void loop()
{
  ...
  else if (motorState == RUNNING_PUMPS)
  {
    ...
    for (int i = 0; i < numPumps; i++)
    {
      ...
      {
        digitalWrite(motorPins[i], LOW);
        delay(100); // Add a small delay to ensure the pumps turn off individually
        pumpStartTimes[i] = 0; // Reset pumpStartTimes for the deactivated pump
      }
    }
